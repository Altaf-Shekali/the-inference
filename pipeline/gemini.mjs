...
export async function geminiJobs({ facts, category, channelName, sourceUrl = "" }) {
  ...
  const user =
    ...
    `- NEVER speak a URL in the vo (the TTS mangles Latin). Say "ಅಫೀಷಿಯಲ್ ವೆಬ್‌ಸೈಟ್" and "ಆ ಲಿಂಕ್ ಅನ್ನ ನಾನು ಡಿಸ್ಕ್ರಿಪ್ಷನ್ ನಲ್ಲಿ ಕೊಟ್ಟಿರುತ್ತೀನಿ" instead — the URL itself goes only in on-screen bullets and the description.\n` +
    ...
    `- The intro vo uses the SIGNATURE GREETING (ನಮಸ್ಕಾರ + ಸ್ವಾಗತ + subscribe/bell + "ಸ್ಕಿಪ್ ಮಾಡದೆ ಪೂರ್ತಿಯಾಗಿ ನೋಡಿ") then states the opportunity + its single most attractive fact. The outro vo uses the SIGNATURE SIGN-OFF (thanks for watching fully + subscribe/bell) AND reminds viewers to verify on the official website before applying.\n\n` +
